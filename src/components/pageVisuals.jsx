...
function Card({ children, className = '' }) {
  return (
    <div className={`relative mx-auto w-full max-w-md overflow-hidden rounded-3xl border border-hairline bg-white/[0.04] p-6 shadow-card backdrop-blur-md ${className}`}>
      {children}
    </div>
  )
}
  ...
  return (
    <Card>
      ...
      <div className="relative mt-4 space-y-2">
        {[0, 1, 2].map((i) => (
          <motion.div key={i} className="flex items-center gap-2.5 rounded-lg border border-hairline bg-white/[0.03] px-3 py-2"
    ...
    <Card>
      ...
      <div className="relative space-y-2.5">
        {['Jordan • VP Sales', 'Priya • Head of Growth', 'Marcus • RevOps Lead'].map((p, i) => (
          <div key={p} className="flex items-center gap-3 rounded-xl border border-hairline bg-white/[0.03] px-3 py-2.5">
    ...
    <Card>
      ...
      <div className="relative mt-4 flex flex-wrap gap-2">
        {['SaaS · B2B', '120 staff', 'Austin, TX', 'Series B'].map((c) => (
          <span key={c} className="rounded-full border border-hairline bg-white/[0.03] px-2.5 py-1 text-[11px] text-ink/80">{c}</span>
        ))}
      </div>
      <div className="relative mt-4 space-y-2">
        {[['Mail', 'verified email'], ['Phone', 'direct dial']].map(([icon, label], i) => (
          <motion.div key={label} className="flex items-center gap-2.5 rounded-lg border border-hairline bg-white/[0.03] px-3 py-2 text-xs text-ink/80"
...
function Cascade({ accent }) {
  ...
  return (
    <Card>
      <Glow accent={accent} />
      <div className="relative mb-4 inline-flex items-center gap-2 rounded-lg border border-hairline bg-white/[0.03] px-3 py-2 text-xs text-ink/80">
...
function DomainCard({ accent }) {
  return (
    <Card>
      <Glow accent={accent} />
      <div className="relative inline-flex items-center gap-2 rounded-lg border border-hairline bg-white/[0.03] px-3 py-2 text-xs text-ink/80">
      ...
      <div className="relative mt-4 grid grid-cols-2 gap-2">
        {[['Industry', 'B2B SaaS'], ['Headcount', '210'], ['Location', 'Berlin'], ['Stack', 'React · AWS']].map(([k, v], i) => (
          <motion.div key={k} className="rounded-xl border border-hairline bg-white/[0.03] px-3 py-2.5"
      ...
      <div className="relative mt-3 flex items-center gap-2.5 rounded-xl border border-hairline bg-white/[0.03] px-3 py-2.5">
...
function PeopleCards({ accent, label = 'verified', title = 'Sourced & verified' }) {
  ...
  return (
    <Card>
      ...
      <div className="relative space-y-2.5">
        {rows.map(([in_, name], i) => (
          <motion.div key={name} className="flex items-center gap-3 rounded-xl border border-hairline bg-white/[0.03] px-3 py-2.5"
    ...
    <Card>
      ...
      <div className="relative grid grid-cols-2 gap-2.5">
        {['Acme Co', 'Globex', 'Initech', 'Umbrella'].map((c, i) => (
          <motion.div key={c} className="rounded-xl border border-hairline bg-white/[0.03] p-3"
    ...
    <Card>
      ...
      <div className="relative space-y-2.5">
        {['Catch-all removed', 'Duplicates merged', 'Emails re-verified', 'Records standardised'].map((t, i) => (
          <motion.div key={t} className="flex items-center justify-between rounded-lg border border-hairline bg-white/[0.03] px-3 py-2 text-xs text-ink/85"
