      ...
      <div className="container-px">
        ...
        <Reveal delay={0.1} className="mt-14">
          <div className="overflow-hidden rounded-2xl border border-hairline bg-panel/70 shadow-card backdrop-blur-md">
            {/* sample-data banner */}
            <div className="flex items-center justify-between gap-3 border-b border-hairline bg-black/[0.03] px-4 py-2.5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1040px] border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-hairline bg-black/[0.02] text-xs uppercase tracking-wide text-muted">
                    {COLUMNS.map((col) => (
                ...
                <tbody>
                  {SAMPLE_LEADS.map((lead, i) => (
                    <tr
                      key={lead.email}
                      className={`border-b border-hairline/60 align-top transition-colors last:border-0 hover:bg-black/[0.025] ${
                        i % 2 ? 'bg-black/[0.012]' : ''
                      }`}
                    >
